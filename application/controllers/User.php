...
// ************* Start common data   ***********
    ...
    public function nwsale()
    {
        ...
        $data['itmDet'] = $this->Generic_model->getData('stck_item', array('itid,icde,itnm,itcd'), array('stat' => 1));

        $this->load->view('modules/user/sales_new', $data);
        $this->load->view('modules/common/footer');
        $this->load->view('modules/common/cus_script_full_width');
    }

    // GET BARCODE DETAILS
    function getBrcdDetils()
    {
        $this->db->select("itid,icde,itnm,itcd,IFNULL(ifwr,0) AS ifwr,wrtp,pird");
        ...
        $this->db->where('stck_item.icde ', $this->input->post('bcde'));
        $query = $this->db->get();
        $data['itdtil'] = $query->result();

        if (!empty($data['itdtil'])) {
            $this->db->select("*");
            $this->db->from("stck_main_sub");
            $this->db->where('stck_main_sub.itid ', $data['itdtil'][0]->itid);
            $this->db->where('stck_main_sub.stat ', 2);
            $query2 = $this->db->get();
            $data['stkpric'] = $query2->result();
        }
        echo json_encode($data);
    }

    // GET ITEM SUB DETAILS
    function getItmSubDetils()
    {
        $this->db->select("*");
        $this->db->from("stck_main_sub_des");
        $this->db->where('stck_main_sub_des.stsb ', $this->input->post('stsbid'));
        $this->db->where('stck_main_sub_des.stat ', 1);
        $query = $this->db->get();
        $data['seriDet'] = $query->result();

        $data['itmPric'] = $this->Generic_model->getData('stck_main_sub', array('csvl,slvl,dsvl'), array('stat' => 2, 'sbid' => $this->input->post('stsbid')));
    ...
    function addInvoice()
    {
        ...
        $brn = $this->input->post('inbrn');

        $brnDt = $this->Generic_model->getData('brch_mas', array('brcd'), array('brid' => $brn));

        // TMP NO GENARATE
        $tmp = "IN/" . $brnDt[0]->brcd . '/' . date('y') . '/';

        $this->db->select("inno");
        $this->db->from("invoice");
        $this->db->where("invoice.inno LIKE '%$tmp%'");
        $this->db->order_by('inid', 'desc');
        $this->db->limit(1);
        $query = $this->db->get();
        $data = $query->result();

        if (count($data) == '0') {
            $spcd = "IN/" . $brnDt[0]->brcd . '/' . date('y') . '/00001';  // Ex SH-001
        } else {
            $spc = $data[0]->inno;
            $re = (explode("/", $spc));
            $aa = intval($re[3]) + 1;
            ...
            $spcd = "IN/" . $brnDt[0]->brcd . '/' . date('y') . '/' . $xx;
        }

        $data_arr = array(
            'brco' => $brn,
            'inno' => $spcd,
            'cunm' => strtoupper($this->input->post('cusNm')),
            'indt' => date('Y-m-d'),
            'inam' => $this->input->post('ttlSubHd'),
            'stat' => 1,
            'pmtp' => 1,
...
//            'dscr' => $this->input->post('remk'),
        ...
        $this->Generic_model->insertData('invoice', $data_arr);

        $addIcd = $this->input->post("addIcd[]");
        $itSril = $this->input->post('itSril[]');
        $itDsvl = $this->input->post('itDsvl[]');
        $itDisc = $this->input->post('itDisc[]');
        $itSlvl = $this->input->post('itSlvl[]');
        $itAdqt = $this->input->post('itAdqt[]');
        $itSbvl = $this->input->post('itSbvl[]');
        $stkid = $this->input->post('stkid[]');

        $invdt = $this->Generic_model->getData('invoice', array('inid'), array('inno' => $spcd));
        $siz = sizeof($addIcd);
        for ($a = 0; $a < $siz; $a++) {

            // if ($subamt[$a] != '' || $subamt[$a] != 0) {

            $itmdt = $this->Generic_model->getData('stck_item', array('itid', 'ifwr', 'wrtp', 'pird'), array('icde' => $addIcd[$a]));
            ...
            $data_arr2 = array(
                ...
                'tdsv' => ($itDsvl[$a] * $itAdqt[$a]),
                'tdis' => ($itDisc[$a] * $itAdqt[$a]),
                'sbtl' => $itSbvl[$a],
                'itsr' => $itSril[$a],

                'wrnt' => ($itmdt[0]->ifwr == 1) ? 1 : 0, // if warranty 1-yes / 0 - No
                'wrtp' => $itmdt[0]->wrtp,
                'wrpr' => $itmdt[0]->pird,
            ...
            $this->Generic_model->insertData('invoice_sub', $data_arr2);
            // }

            // STOCK SOLD ITEM STATUES CHANGE
            $re = (explode(",", $itSril[$a]));
            if (sizeof($re) > 0) {
                for ($x = 0; $x < sizeof($re); $x++) {
                    $this->Generic_model->updateData('stck_main_sub_des', array('stat' => 2, 'inid' => $invdt[0]->inid), array('dtid' => $re[$x]));
                }
            }

            // AVALIBLE QUENTY UPDATE
            $stkdt = $this->Generic_model->getData('stck_main_sub', array('avqn'), array('sbid' => $stkid[$a]));
            if ($stkdt[0]->avqn > 0) {
                $this->Generic_model->updateData('stck_main_sub', array('avqn' => ($stkdt[0]->avqn - $itAdqt[$a])), array('sbid' => $stkid[$a]));
            }
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            //echo json_encode(true);
            echo json_encode($invdt[0]->inid);

        }
        $funcPerm = $this->Generic_model->getFuncPermision('nwsale');
        $this->Log_model->userFuncLog($funcPerm[0]->pgid, 'Add New Invoice (' . $spcd . ')');
    }

    // PRINT INVOICE
    function invoicPrint($inid)
    {
        $rcdt = $this->Generic_model->getData('invoice', array('pntc'), array('inid' => $inid));
        $pntc = $rcdt[0]->pntc + 1;

        // PRINT COUNT UPDATE INVOICE TB
        if (($pntc - 1) > 0) {
            $data_ar1 = array(
        ...
        $result1 = $this->Generic_model->updateData('invoice', $data_ar1, array('inid' => $inid));

        //$lndt = $this->Generic_model->getData('micr_crt', array('lnid', 'vpno', 'stat'), array('vpno' => $vuid));

        if (count($result1) > 0) {
            $this->load->library('ciqrcode');
            $this->db->select("invoice.*,brch_mas.brnm, user_mas.fnme");
            $this->db->from("invoice");
            $this->db->join('brch_mas', 'brch_mas.brid = invoice.brco ');
            $this->db->join('user_mas', 'user_mas.auid = invoice.crby ');
            //$this->db->join('invoice_sub', 'invoice_sub.inid = invoice.inid ');
            //$this->db->join('pay_terms', 'pay_terms.tmid = invoice.pmtp ');
            ...
            // INVOICE SUB DETAILS
            //$insbDt = $this->Generic_model->getData('invoice_sub', '', array('inid' => $inid));
            $insbDt = $this->Generic_model->getJoin('invoice_sub', '', array('inid' => $inid), 'stck_item', 'itid', 'itid', '', '');
            ...
            $this->pdf->Cell(0, 0, 'CASH INVOICE', 0, 1, 'C');
            ...
            $this->pdf->SetXY(6, 45);
            $this->pdf->Cell(1, 0, 'CUSTOMER : ' . $data[0]->cunm, 0, 1, 'L');
            ...
            $this->pdf->Cell(1, 0, 'NO :' . $data[0]->inno, 0, 1, 'L');
            $this->pdf->SetXY(110, 49);
            $this->pdf->Cell(1, 0, 'DATE : ' . $data[0]->indt, 0, 1, 'L');
            $this->pdf->SetXY(110, 53);
            $this->pdf->Cell(1, 0, 'BRANCH :' . $data[0]->brnm, 0, 1, 'L');
            /*$this->pdf->SetXY(158.5, 45);
            $this->pdf->Cell(1, 0, ': ' . $data[0]->inno, 0, 1, 'L');
            ...
            $this->pdf->SetXY(5, 37);
            $this->pdf->Cell(140, 30, '', '1');

            // Payment details table border
            $this->pdf->SetXY(5, 60);
            $this->pdf->Cell(15, 80, '', '1');
            $this->pdf->SetXY(20, 60);
            $this->pdf->Cell(70, 80, '', '1');
            $this->pdf->SetXY(90, 60);
            $this->pdf->Cell(15, 80, '', '1');
            $this->pdf->SetXY(105, 60);
            $this->pdf->Cell(20, 80, '', '1');
            $this->pdf->SetXY(125, 60);
            $this->pdf->Cell(20, 80, '', '1');

            // #0
            $this->pdf->SetXY(5, 60);
            $this->pdf->Cell(15, 7, 'ITEM', 1, 1, 'C');
            $this->pdf->SetXY(20, 60);
            $this->pdf->Cell(70, 7, 'DESCRIPTION', 1, 1, 'C');
            $this->pdf->SetXY(90, 60);
            $this->pdf->Cell(15, 7, 'QTY', 1, 1, 'C');
            $this->pdf->SetXY(105, 60);
            $this->pdf->Cell(20, 7, 'UNIT PRICE', 1, 1, 'C');
            $this->pdf->SetXY(125, 60);
            $this->pdf->Cell(20, 7, 'SUB TTL', 1, 1, 'C');
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                if ($insbDt[$i]->wrnt == 1) {
                    if ($insbDt[$i]->wrtp == 1) {
                        $pr = " Days";
                    } else if ($insbDt[$i]->wrtp == 2) {
                        $pr = " Weeks";
                    } else if ($insbDt[$i]->wrtp == 3) {
                        $pr = " Months";
                    } else if ($insbDt[$i]->wrtp == 4) {
                        $pr = " Years";
                    }
                    $wrn = ' (' . $insbDt[$i]->wrpr . $pr . ' Warranty)';
                } else {
                    $wrn = '';
                }

                $this->pdf->SetXY(5, $y);
                $this->pdf->Cell(15, 0, $insbDt[$i]->bcde, '0', '', 'C');
                $this->pdf->SetXY(20, $y);
                $this->pdf->Cell(70, 0, $insbDt[$i]->itcd . ' - ' . $insbDt[$i]->itnm, '0');
                if ($insbDt[$i]->wrnt == 1) {
                    $this->pdf->SetXY(20, $y + 3);
                    $this->pdf->Cell(70, 0, $wrn, '0');
                }
                $this->pdf->SetXY(90, $y);
                $this->pdf->Cell(15, 0, $insbDt[$i]->qunt, '', '', 'R');
                $this->pdf->SetXY(105, $y);
                $this->pdf->Cell(20, 0, number_format($insbDt[$i]->slvl, 2, '.', ','), '0', '', 'R');
                $this->pdf->SetXY(125, $y);
                $this->pdf->Cell(20, 0, number_format($insbDt[$i]->sbtl, 2, '.', ','), '0', '', 'R');
                $y = $y + 5;
                $pyamt = $pyamt + $insbDt[$i]->sbtl;
            }
            ...
            $this->pdf->SetFont('Helvetica', 'B', 8);
            $this->pdf->SetXY(90, 140);
            $this->pdf->Cell(6, 10, 'TOTAL AMOUNT', 0, 1, 'R');
            $this->pdf->SetXY(125, 144);
            $this->pdf->Cell(20, 0, number_format($pyamt, 2, '.', ','), '0', '', 'R');
            $this->pdf->SetAutoPageBreak(false);

            $this->pdf->line(5, 160, 145, 160);
            // SERVICE MENUS LEFT
            //$this->pdf->SetXY(5, 160);
            //$this->pdf->Cell(140, 30, '', '1');
            $this->pdf->SetXY(5, 160);
            $this->pdf->Cell(70, 7, 'Repair services', 0, 1, 'L');
            $this->pdf->SetFont('Helvetica', '', 7);
            $this->pdf->SetXY(5, 165);
            $this->pdf->Cell(70, 7, '* Mobile Repairing', 0, 1, 'L');
            $this->pdf->SetXY(5, 168);
            $this->pdf->Cell(70, 7, '* Software Update', 0, 1, 'L');
            $this->pdf->SetXY(5, 171);
            $this->pdf->Cell(70, 7, '* Touch Replaced', 0, 1, 'L');
            $this->pdf->SetXY(5, 174);
            $this->pdf->Cell(70, 7, '* Mobile Unlock', 0, 1, 'L');
            $this->pdf->SetXY(5, 177);
            $this->pdf->Cell(70, 7, '* Reprogramming', 0, 1, 'L');

            // accessories
            // SERVICE MENUS RIGHT
            //$this->pdf->SetXY(75, 160);
            //$this->pdf->Cell(70, 30, '', '1');
            $this->pdf->SetXY(75, 160);
            $this->pdf->SetFont('Helvetica', 'B', 8);
            $this->pdf->Cell(70, 7, 'Accessories', 0, 1, 'L');
            $this->pdf->SetFont('Helvetica', '', 7);
            $this->pdf->SetXY(75, 165);
            $this->pdf->Cell(70, 7, '* Any mobile device', 0, 1, 'L');
            $this->pdf->SetXY(75, 168);
            $this->pdf->Cell(70, 7, '* Phone case', 0, 1, 'L');
            $this->pdf->SetXY(75, 171);
            $this->pdf->Cell(70, 7, '* Screen protectors', 0, 1, 'L');
            $this->pdf->SetXY(75, 174);
            $this->pdf->Cell(70, 7, '* Charges, Cables', 0, 1, 'L');
            $this->pdf->SetXY(75, 177);
            $this->pdf->Cell(70, 7, '* Hand frees & Audios', 0, 1, 'L');

            $this->pdf->line(5, 185, 145, 185);
            //Line(float x1, float y1, float x2, float y2)


            $this->pdf->SetFont('Helvetica', '', 7);
            $this->pdf->SetXY(4, 195);
            ...
            $this->pdf->Cell(10, 6, 'Copyright@' . $cy . ' - www.gemusolution.com', 0, 1, 'R');
            $this->pdf->SetXY(4, 200);
            ...
            //QR CODE
            $cd = 'Vou No : ' . $data[0]->inno . ' | Date : ' . $data[0]->crdt . ' | Payee Name : ' . $data[0]->cunm . ' | Branch : ' . $data[0]->brnm . ' | Pay Type : ' . '$data[0]->dsnm' . ' | Total : Rs.' . $pyamt . ' | Printed By : ' . $usr . ' | ' . $_SESSION["hid"];
            $this->pdf->Image(str_replace(" ", "%20", 'http://chart.apis.google.com/chart?cht=qr&chs=190x190&chl=' . $cd), 120, 5, 26, 0, 'PNG');

            $this->pdf->SetTitle('Cash Invoice - ' . $data[0]->inno);
            $this->pdf->Output('Cash_Invoice_' . $data[0]->inno . '.pdf', 'I');
            ob_end_flush();
        ...
        $funcPerm = $this->Generic_model->getFuncPermision('nwsale');
        $this->Log_model->userFuncLog($funcPerm[0]->pgid, 'Invoice print (' . $data[0]->inno . ')');
    }
//
//
// ITEM MANAGEMENT
    function todysale()
    {
        ...
        $data['funcPerm'] = $this->Generic_model->getFuncPermision('todysale');
        $this->load->view('modules/user/today_sales', $data);
        $this->load->view('modules/common/footer');
        $this->load->view('modules/common/cus_script_full_width');
    ...
    function getSales()
    {
        $funcPerm = $this->Generic_model->getFuncPermision('todysale');
        ...
        $result = $this->User_model->get_tdySles();
        $data = array();
        $i = $_POST['start'];
        foreach ($result as $row) {
            ...
            $inid = $row->inid;
            $option =
                "<button type='button' $viw  data-toggle='modal' data-target=''  onclick='vewInvo($inid);' class='btn btn-sm btn-default btn-condensed' title='view' ><i class='fa fa-eye' aria-hidden='true'></i></button> " .
                "<button type='button' id='edt'  $edt  data-toggle='modal' data-target=''  onclick='reprint($inid);' class='btn btn-sm btn-default btn-condensed' title='Reprint' ><i class='fa fa-print' aria-hidden='true'></i></button> " ;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->User_model->count_all_sles(),
            "recordsFiltered" => $this->User_model->count_filtered_sles(),
            "data" => $data,
        );
