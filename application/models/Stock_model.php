...
class Stock_model extends CI_Model
{
    ...
    //  SUPPLY MANAGEMENT
    var $cl_srch1 = array('brnm', 'usnm', 'fnme', 'almo', 'unic', 'lvnm',); //set column field database for datatable searchable
    var $cl_odr1 = array(null, 'spcd', '', 'usnm', 'fnme', 'almo', 'unic', 'lvnm', '', '', '', 'stat', '', ''); //set column field database for datatable orderable
    var $order1 = array('stat' => 'asc'); // default order

    function supply_query()
    {
        $this->db->select("stck_supply.*,CONCAT(user_mas.usnm) AS exe, bnk_names.bknm");
        $this->db->from("stck_supply");
        $this->db->join('user_mas', 'user_mas.auid = stck_supply.crby');
        $this->db->join('bnk_names', 'bnk_names.bkid = stck_supply.bkid', 'left');

    }

    private function supply_queryData()
    {
        $this->supply_query();
        $i = 0;
...
//  WAREHOUSE MANAGEMENT
    var $cl_srch5 = array('brnm', 'usnm', 'fnme', 'almo', 'unic', 'lvnm',); //set column field database for datatable searchable
    var $cl_odr5 = array(null, 'whcd', '', 'usnm', 'fnme', 'almo', 'unic', 'lvnm', '', '', '', 'stat', '', ''); //set column field database for datatable orderable
    var $order5 = array('stat' => 'asc'); // default order

    function warehouse_query()
    {
        $this->db->select("stck_warehouse.*,CONCAT(user_mas.usnm) AS exe");
        $this->db->from("stck_warehouse");
        $this->db->join('user_mas', 'user_mas.auid = stck_warehouse.crby');
    }

    private function warehouse_queryData()
    {
        $this->warehouse_query();
        $i = 0;
...
//
// HIRE ITEM
    var $cl_srch2 = array('ctnm', 'brnm', 'mdnm', 'tpnm', 'itnm', 'itcd'); //set column field database for datatable searchable
    var $cl_odr2 = array(null, 'ctnm', 'brnm', 'mdnm', 'tpnm', 'itnm', 'itcd', 'usnm', 'crdt', 'stat', ''); //set column field database for datatable orderable
    var $order2 = array('crdt' => 'desc'); // default order

    function hireItem_query()
    {
        $srct = $this->input->post('srct'); // CATEGORY
        $srbr = $this->input->post('srbr'); // BRAND
        $srmd = $this->input->post('srmd'); // MODEL
        ...
        if ($srct != 'all') {
            $this->db->where('stck_item.ctid', $srct);
        }
        if ($srbr != 'all') {
            $this->db->where('stck_item.brid', $srbr);
        }
        if ($srmd != 'all') {
            $this->db->where('stck_item.mdid', $srmd);
        }
        if ($stat != 'all') {
            $this->db->where('stck_item.stat', $stat);
        }
        //$this->db->order_by('micr_crt.lnid', 'DESC'); //ASC  DESC
    }

    private function hireItem_queryData()
    {
        $this->hireItem_query();
        $i = 0;
    ...
    function get_hireItem()
    {
        $this->hireItem_queryData();
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    function count_filtered_hireItem()
    {
        $this->hireItem_queryData();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_hireItem()
    {
        $this->hireItem_query();
        return $this->db->count_all_results();
    }
...
//
// STOCK
    var $cl_srch3 = array('ctnm', 'brnm', 'mdnm', 'tpnm', '', 'itcd'); //set column field database for datatable searchable
    var $cl_odr3 = array(null, 'ctnm', 'brnm', 'mdnm', 'tpnm', '', '', 'usnm', 'crdt', 'stat', ''); //set column field database for datatable orderable
    var $order3 = array('crdt' => 'desc'); // default order

    function stock_query()
    {
        $sply = $this->input->post('sply'); // SUPPLY
        $frdt = $this->input->post('frdt'); // FROM
        $todt = $this->input->post('todt'); // TO

        $this->db->select("stck_main.*, stck_supply.spnm, CONCAT(user_mas.usnm) AS exc, DATE_FORMAT(stck_main.crdt, '%Y-%m-%d') AS crdt");
        $this->db->from("stck_main");

        //$this->db->join('stck_item', 'stck_item.itid = stck_main.itid ');
        $this->db->join('stck_supply', 'stck_supply.spid = stck_main.spid ');
        $this->db->join('user_mas', 'user_mas.auid = stck_main.crby ');

        if ($sply != 'all') {
            $this->db->where('stck_main.spid', $sply);
        }
        $this->db->where("DATE_FORMAT(stck_main.crdt, '%Y-%m-%d') BETWEEN '$frdt' AND '$todt'");
    }

    private function stock_queryData()
    {
        $this->stock_query();
        $i = 0;
...
//
// STOCK LIST
    var $cl_srch4 = array('stno', 'itnm', 'qnty', 'avqn', 'csvl', 'slvl', 'dsvl', 'sbtt', 'crdt', 'crby', ''); //set column field database for datatable searchable
    var $cl_odr4 = array(null, 'stno', 'itnm', 'qnty', 'avqn', 'csvl', 'slvl', 'dsvl', 'sbtt', 'crdt', 'crby', ''); //set column field database for datatable orderable
    var $order4 = array('crdt' => 'desc'); // default order

    function stockList_query()
    {
        $sply = $this->input->post('sply'); // SUPPLY
        $frdt = $this->input->post('frdt'); // FROM
        $todt = $this->input->post('todt'); // TO
        ...
        $this->db->where('stck_main_sub.stat != 0');    // != inactive
    ...
    private function stockList_queryData()
    {
        $this->stockList_query();
        $i = 0;
        foreach ($this->cl_srch3 as $item) // loop column
        {
            if ($_POST['search']['value']) // if datatable send POST for search
    ...
    function count_filtered_stockList()
    {
        $this->stockList_queryData();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_stockList()
    {
        $this->stockList_query();
        return $this->db->count_all_results();
    }
...
//
    ...
    // center leader
    var $cl_srch44 = array('cnnm'); //set column field database for datatable searchable
    var $cl_odr44 = array(null, 'cnnm', 'grno', 'cuno', 'init', 'mobi', 'cutp', ''); //set column field database for datatable orderable
    var $order44 = array('cnnm' => 'asc'); // default order
