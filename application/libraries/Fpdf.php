...
// VoucherTable PDF table
    function VoucherTable($header, $data)
    {
        // Column widths
        $w = array(10, 23, 35, 30, 20, 20, 20, 10, 20, 20, 15, 15, 18, 30);
        ...
        foreach ($data['aa'] as $row) {
            ...
            $this->Cell($w[1], 6, $row->vuno, 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row->lnno, 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row->title_name . $row->cust_name_short, 'LR', '', 'L', $fill);
            $this->Cell($w[4], 6, $row->cust_id, 'LR', 0, 'L', $fill);
            $this->Cell($w[5], 6, $row->cust_nic_no, 'LR', '', 'L', $fill);
            $this->Cell($w[6], 6, $row->center_name, 'LR', '', 'L', $fill);
            $this->Cell($w[7], 6, $row->product_code, 'LR', 0, 'L', $fill);
            $this->Cell($w[8], 6, number_format($row->lnamnt, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[9], 6, number_format($row->vuam, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[10], 6, number_format($row->doccrg, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[11], 6, 'INS', 'LR', 0, 'L', $fill);
            $this->Cell($w[12], 6, $row->username, 'LR', 0, 'L', $fill);
            $this->Cell($w[13], 6, $row->crdt, 'LR', 0, 'L', $fill);
            $this->Ln();
            $fill = !$fill;
            $i++;
...
// Debit Receipts PDF table
    function dreceiptsTable($header, $data)
    {
        // Column widths
        $w = array(10, 25, 35, 30, 25, 20, 25, 10, 20, 20, 35, 25);
        // Colors, line width and bold font
        $this->SetFillColor(100, 100, 100);
        $this->SetTextColor(255);
        $this->SetDrawColor(0, 0, 0);
        ...
        foreach ($data['dr'] as $row) {
            $this->Cell($w[0], 6, $i, 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, $row->reno, 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row->rfno, 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row->title_name . $row->cust_name_short, 'LR', '', 'L', $fill);
            $this->Cell($w[4], 6, $row->cusid, 'LR', 0, 'L', $fill);
            $this->Cell($w[5], 6, $row->cust_nic_no, 'LR', '', 'L', $fill);
            $this->Cell($w[6], 6, $row->center_name, 'LR', '', 'L', $fill);
            $this->Cell($w[7], 6, $row->product_code, 'LR', 0, 'L', $fill);
            $this->Cell($w[8], 6, number_format($row->curr_tot_balance, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[9], 6, number_format($row->ramt, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[10], 6, $row->crdt, 'LR', 0, 'L', $fill);
            $this->Cell($w[11], 6, $row->first_name . '  ' . $row->last_name, 'LR', 0, 'L', $fill);
//            $this->Cell($w[9], 6, number_format($row->doccrg, 2), 'LR', '', 'R', $fill);
//            $this->Cell($w[10], 6, 'INS', 'LR', 0, 'L', $fill);
            $this->Ln();
            $fill = !$fill;
            $i++;
...
// Cash book PDF table
    function cashbookTable($header, $data)
    {
        $this->SetX(40);
        // Column widths
        $w = array(10, 35, 90, 30, 30, 30);
        // Colors, line width and bold font
        $this->SetFillColor(100, 100, 100);
        $this->SetTextColor(255);
        $this->SetDrawColor(0, 0, 0);
        ...
        $balance = 0;
        $tot_bal = 0;
        foreach ($data['cash'] as $row) {

            $balance = $balance + $row->dbam - $row->cram;
            $credet = $credet + $row->cram;
            $debit = $debit + $row->dbam;

            $this->SetX(40);
            $this->Cell($w[0], 6, $i, 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, $row->acdt, 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row->trtp . ' - ' . $row->dcrp . ' - ' . $row->acst, 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, number_format($row->dbam, 2), 'LR', '', 'R', $fill);
            $this->Cell($w[4], 6, number_format($row->cram, 2), 'LR', 0, 'R', $fill);
            $this->Cell($w[5], 6, number_format($balance, 2), 'LR', 0, 'R', $fill);
        ...
        $this->Cell($w[3], 6, number_format($credet, 2), 'LR', '', 'R', $fill);
        $this->Cell($w[4], 6, number_format($debit, 2), 'LR', 0, 'R', $fill);
        $this->Cell($w[5], 6, number_format($tot_bal, 2), 'LR', 0, 'R', $fill);
